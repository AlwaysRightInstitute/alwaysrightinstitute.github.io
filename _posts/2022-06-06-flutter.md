...
This will start the simulator, but not build the app yet.
...
It can be changed to look a little more like UIKit, we'll get to that.
...
Flutter itself is a framework on top. The key concept of Flutter is a
...
but for those you still don't put
the state _inside_ the Widget, but into a separate, `State<Widget>` object.
Pretty similar to our
[ViewController](http://www.alwaysrightinstitute.com/viewcontroller/).
<br>
Speaking of objects, everything in Dart is classes and objects, no structs.


## Comparing the HelloWorld App

In the [introduction](#introduction) we've shown the boilerplate app
`flutter create` generates. It is a simple screen with a Text showing
a counter, and a button to increase that counter.
...
Note how the SwiftUI app looks proper by default and the Flutter app looks
like an Android thing. We'll get to that later.
...
It's just a `main` function in Dart
...
while in SwiftUI you can intermingle View "configuration" and its "state"
in a single entity. 
...
once SwiftUI apps start to grow.
...
[ViewController](http://www.alwaysrightinstitute.com/viewcontroller/)
approach.
`State` is such a ViewController, it is a very specific `ObservableObject`
that has a View attached, which is being controlled.
...
`@State` essentially turns the View "struct" into an object 
...
[`@State`](https://developer.apple.com/documentation/swiftui/state)
property wrapper sees the modification and automatically marks the View for
re-evaluation 
...
- Flutter doesn't usually use "View Modifiers" like `.padding` (though we think
  that would be possible, it is just not idiomatic),
  but rather wraps Widgets in Widgets in Widgets.
...
the arguments of the widget "init call".
...
We think those two things are where Flutter doesn't flutter by really flies.
...
(which we dislike, code generation is wrong™️).
...
```
And et voilà (again, just `⌘s` to save _and_ live-deploy!):
...
(did we mention: Flutter documentation 😚).
...
If we would be forced (💰💰💰💰💰) to produce an Android app,
...
- Wikipedia on [Flutter](https://en.wikipedia.org/wiki/Flutter_(software))
- Wikipedia on [Dart](https://en.wikipedia.org/wiki/Dart_(programming_language))
- [Visual Studio Code](https://code.visualstudio.com):
